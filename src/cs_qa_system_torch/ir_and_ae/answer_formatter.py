...
def reformat_answer(text, help_page_url, title, max_ans_len=247):
    ...
    while j < len(answer_list):
        ...
        if len(re.sub('<a>|</a>', '', ans)) + len(re.sub('<a>|</a>', '', next_ans)) < max_ans_len and url_check == 0:
            ans += '. ' + next_ans
            url += next_url
    ...
    if len(formatted_answer) > 2:
        ...
        formatted_answer[1][0] = formatted_answer[1][0] + '...'

    open_bubble = ["I found some information from one of our Amazon Help pages that may be able to help.", ""]
    close_bubble = ["To learn more, check out our <a>{} help page</a>.".format(title), help_page_url]
